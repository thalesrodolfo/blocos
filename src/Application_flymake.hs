...
app' pool = do
  S.middleware (logStdoutDev . allowCsrf . corsified)
  ...
  S.post "/api/v1/loginUser" $ do
    (u :: LoginUser) <- S.jsonData
    loggedUser <- liftIO $ loginUser (loginUsername u) (loginPassword u) pool
    case loggedUser of
      Nothing -> S.status status400
