...
var author$project$Main$Model = F2(
	function (loginForm, formState) {
		return {formState: formState, loginForm: loginForm};
	});
var author$project$Main$ShowForm = {$: 'ShowForm'};
...
var author$project$Main$GotValidation = function (a) {
	return {$: 'GotValidation', a: a};
};
	...
	return elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'loginUsername',
				elm$json$Json$Encode$string(model.loginForm.username)),
				_Utils_Tuple2(
				'loginPassword',
				elm$json$Json$Encode$string(model.loginForm.password))
			]));
};
...
var author$project$Main$validateUser = function (model) {
	var body = elm$http$Http$jsonBody(
		author$project$Main$userEncoder(model));
	return elm$http$Http$post(
		{
			body: body,
			expect: A2(elm$http$Http$expectJson, author$project$Main$GotValidation, author$project$Main$userDecoder),
			...
			case 'Logar':
				return _Utils_Tuple2(
					A2(
						...
						_Utils_update(
							model,
							{formState: author$project$Main$Loading})),
					author$project$Main$validateUser(model));
			case 'SetUsername':
				var newUsernameValue = msg.a;
				var oldForm = model.loginForm;
			...
			default:
				...
				if (result.$ === 'Ok') {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{formState: author$project$Main$Success}),
						elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{formState: author$project$Main$Failure}),
						elm$core$Platform$Cmd$none);
				}
		}
	});
var author$project$Main$Logar = {$: 'Logar'};
...
var author$project$Main$viewForm = A2(
	...
	_List_fromArray(
		[
			A2(elm$html$Html$Attributes$style, 'padding-top', '100px')
		]),
	_List_fromArray(
		[
			A2(
				...
				[
					...
					_List_fromArray(
						[
							...
							elm$html$Html$Events$onClick(author$project$Main$Logar)
						]),
					_List_fromArray(
						[
							elm$html$Html$text('Logar')
						]))
				]))
		]));
var author$project$Main$viewGif = function (model) {
	var _n0 = model.formState;
	switch (_n0.$) {
		case 'Failure':
			return A2(
				elm$html$Html$div,
				...
				_List_fromArray(
					[
						elm$html$Html$text('Erro: Não foi possível logar com o usuário informado. ')
					]));
		case 'Loading':
			return A2(
				elm$html$Html$div,
				_List_fromArray(
					[
						elm$html$Html$Attributes$class('loading loading-lg')
					]),
				_List_Nil);
		case 'Success':
			return A2(
				...
				_List_fromArray(
					[
						elm$html$Html$text('Logado com sucesso')
					]));
		default:
			return author$project$Main$viewForm;
	}
};
var author$project$Main$view = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				elm$html$Html$Attributes$class('column col-2 col-mx-auto flex-centered')
			]),
		_List_fromArray(
			[
